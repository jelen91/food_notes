...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!(await enforceRateLimit(req, res, { key: `claim:${clientIp(req)}`, max: 120, windowSeconds: 3600 }))) return;
  ...
  try {
    ...
    if (!checkout || checkout.draftId !== draftId) return res.status(403).json({ error: 'Platbu se nepodařilo ověřit.' });
    ...
    if (!account || account.status === 'deleted') return res.status(403).json({ error: 'Účet není dostupný.' });
